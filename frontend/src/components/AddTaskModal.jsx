...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`http://localhost:4000/project/${projectId}/task/${taskId}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`http://localhost:4000/project/${projectId}/task`, { title, description: desc })
                .then((res) => {
            ...
            axios.put(`http://localhost:4000/project/${projectId}/task/${taskId}`, { title, description: desc })
                .then((res) => {
                    setAddTaskModal(false)
